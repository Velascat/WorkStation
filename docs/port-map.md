...
|-------------|-------|----------|--------------------------------------------------|
| SwitchBoard | 20401 | TCP/HTTP | API gateway — all inbound client model requests  |
| 9router     | 20128 | TCP/HTTP | Model dispatcher — internal routing to providers |
...
- To change a host port without editing compose files, set the corresponding variable in `.env`:
  ...
  PORT_STATUS=20400
  ```
- Internal service-to-service traffic (e.g. SwitchBoard → 9router) uses Docker bridge network DNS and does not traverse host ports.
